...
# # Initialize Gemini – gemini-2.0-flash for fast, reliable intent parsing
...
# # Helper to generate trace entries following required schema
# def _make_trace(step_name: str, agent_name: str, description: str, input_data: Any, output_data: Any, reasoning: str, tool_used: str, status: str, request_id: str) -> dict:
#     return {
#         "step_name": step_name,
...
#     """Rule-based keyword matching – fallback when LLM quota is exceeded."""
...
#         ServiceType.AC_TECHNICIAN: ["ac", "thanda", "cooling", "gas", "split", "cooler", "ac repair", "ac technician", "ac band"],
...
#     urgency = "medium"
#     if any(word in query_lower for word in ["jaldi", "fauran", "foran", "emergency", "urgent", "abhi", "abhi chahiye",
#                                             "sparks", "leak ho raha", "short circuit", "aag", "fire", "flood",
#                                             "turant", "immediately", "right now", "asap"]):
#         urgency = "high"
#     elif any(word in query_lower for word in ["kal", "tomorrow", "next week", "baad mein", "agli baar",
#                                             "is hafte", "this week", "next month", "plan"]):
#         urgency = "low"

#     urdu_markers = ["hai", "mein", "kar", "ho", "ki", "ka", "chahiye", "bhejien", "parhanay", "subah", "kal", "mujhe", "aur", "se"]
...
#     # START trace for intent understanding
#     start_trace = _make_trace(
#         step_name="intent_understanding",
#         agent_name="graph",
#         description="Parse intent and extract entities",
#         input_data={"raw_query": state["request"].raw_query},
#         output_data=None,
#         reasoning="",
#         tool_used="",
#         status="started",
...
#     )
#     state["trace"] = state.get("trace", []) + [start_trace]

#     """[NODE: Intent Parser] Gemini-2.0-flash + rule-based fallback."""
...
#         llm_log = f"⚠️ [Gemini Failed] → Rule-based fallback activated"
...
#     logger.info(f"Intent parsing: {intent_obj.value}, {lang_val}, {urgency_val}")
...
#     """[NODE: Discovery] Finds available providers within MAX_DISTANCE_KM and calculates proximity."""
#     # START trace for provider search
#     start_trace = _make_trace(
#         step_name="provider_search",
#         agent_name="graph",
#         description="Discover providers within max distance",
#         input_data={"intent": state.get("intent").value if state.get("intent") else None,
#                     "location": state["request"].location.model_dump() if state["request"].location else None},
#         output_data=None,
#         reasoning="",
#         tool_used="",
#         status="started",
...
#             skipped_distant.append((p.name, dist))
#             continue
#         p_dict = p.model_dump()
...
#         discovery_msg = f"⚠️ [Discovery] No available providers found within {MAX_DISTANCE_KM}km for {intent.value}."
#     else:
#         discovery_msg = f"📍 Found {len(providers_with_distance)} available providers within {MAX_DISTANCE_KM}km."

#     logger.info(f"Discovery: {len(providers_with_distance)} providers found")

#     # END trace for provider search
#     end_trace = _make_trace(
#         step_name="provider_search",
#         agent_name="graph",
#         description="Discover providers within max distance",
#         input_data=None,
#         output_data={"providers_count": len(providers_with_distance), "skipped_unavailable": skipped_unavailable, "skipped_distant": skipped_distant},
#         reasoning=discovery_msg,
#         tool_used="DB+Maps",
#         status="completed",
#         request_id=state.get("request_id", "")
#     )
#     state["trace"] = state.get("trace", []) + [end_trace]
...
#     # START trace for provider ranking
#     start_trace = _make_trace(
#         step_name="provider_ranking",
#         agent_name="graph",
#         description="Rank providers based on weighted criteria",
#         input_data={"providers_count": len(state.get("providers", [])), "urgency": state.get("urgency")},
#         output_data=None,
#         reasoning="",
#         tool_used="",
#         status="started",
...
#     logger.info("[Node] Ranking started – providers=%d, urgency=%s" % (len(state.get('providers', [])), state.get('urgency')))
...
#     scored_providers = []
#     for p in providers:
#         rating_score = p["rating"] * rating_weight
#         distance_score = -(p["distance"] * dist_weight)
#         exp_score = p.get("experience_years", 0) * exp_weight
#         total_score = rating_score + distance_score + exp_score
...
#     score_log = " | ".join([
#         f"{p['name']}: rating={p['rating']} dist={p['distance']}km exp={p.get('experience_years', 0)}y score={p['score']}"
#         for p in ranked
#     ])

#     reasoning = (f"{selected['name']} is the top match with a rating of {selected['rating']} "
#                  f"and is located just {selected['distance']}km away.")
#     if urgency in ["high", "emergency"]:
#         reasoning += " Priority given to fast response time due to high urgency."
#     else:
...
#             },
#             "score": p["score"],
#             "reasoning": f"Score {p['score']}: Rating {p['rating']}, Distance {p['distance']}km, Experience {p.get('experience_years', 0)} years"
#         })

#     # END trace for provider ranking
#     end_trace = _make_trace(
#         step_name="provider_ranking",
#         agent_name="graph",
#         description="Rank providers",
#         input_data=None,
#         output_data={"selected_provider": selected["name"], "score": selected["score"]},
#         reasoning=reasoning,
#         tool_used="",
#         status="completed",
#         request_id=state.get("request_id", "")
...
#     # START trace for booking simulation
#     start_trace = _make_trace(
#         step_name="booking_simulation",
#         agent_name="graph",
#         description="Simulate booking creation",
#         input_data={"provider_id": state.get("selected_provider", {}).get("id"), "urgency": state.get("urgency")},
#         output_data=None,
#         reasoning="",
#         tool_used="",
#         status="started",
...
#             step_name="booking_simulation",
#             agent_name="graph",
#             description="Booking simulation failed",
#             input_data=None,
#             output_data=None,
#             reasoning="No viable provider",
#             tool_used="",
#             status="completed",
#             request_id=state.get("request_id", "")
#         )
...
#     logger.info(f"Booking created: {booking_id} for provider {selected['name']}")

#     # END trace for booking simulation
#     end_trace = _make_trace(
#         step_name="booking_simulation",
#         agent_name="graph",
#         description="Booking simulation completed",
#         input_data=None,
#         output_data={"booking_id": booking_id, "scheduled_at": scheduled_at.isoformat()},
#         reasoning=f"Booked with provider {selected['name']}",
#         tool_used="DB",
#         status="completed",
#         request_id=state.get("request_id", "")
#     )
...
#     return {"booking": booking, "trace": state["trace"]}


# def followup_node(state: AgentState):
#     """[NODE: Follow-up] Fully structured JSON — frontend-parseable."""
...
#         input_data=None,
#         output_data=None,
#         reasoning=f"No {service_name} provider available.",
#         tool_used="",
#         status="completed",
#         request_id=state.get("request_id", "")
#     )
...
#             if 5 <= hour < 12:
#                 time_desc = "Tomorrow morning"
#             elif 12 <= hour < 17:
#                 time_desc = "Tomorrow afternoon"
#             else:
#                 time_desc = "Tomorrow evening"
#         else:
...
#                     "description": "Service ke liye relevant area clear karein taake kaam jaldi ho.",
#                     "action_type": "info"
#                 },
#                 {
#                     "step_number": 3,
...
# def complete_booking_node(state: AgentState):
#     """[NODE: Booking Completion] Mark booking as completed and update follow-up."""
#     booking = state.get("booking")
#     if not booking:
#         return {"trace": state["trace"] + ["⚠️ [Completion] No active booking to complete."]}
...
# # Also allow standalone completion from outside
